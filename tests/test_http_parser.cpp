...
const std::string TEST_POST_REQ2 = "Hgin TTP/1.0\r\nContent-Type: application/x-www-form-"
								   "urlencoded\r\n"
								   "Content-Length: 14\r\n"
								   "\r\n"
								   "name=aa&pwd=xx";
...
{	
	...
	int nparsed = http_parser_execute(&parser, &settings, TEST_GET_REQ.c_str(), TEST_GET_REQ.size());
	LOG_INFO("parsed size: %d, parser->upgrade: %d, total_size: %u", nparsed, parser.upgrade, TEST_GET_REQ.size());
	if(parser.http_errno) {
		LOG_INFO("ERROR: %s", http_errno_description(HTTP_PARSER_ERRNO(&parser)));
	}
	return 0;
...
int test_parse_post() 
{
	...
	if(ret != 0) {
		LOG_ERROR("PARSE request error whick ret: %d, req str: %s", ret, TEST_POST_REQ.c_str());
		return ret;
	}
...
int test_get_unescape()
{
	...
	if(ret != 0) {
		LOG_ERROR("PARSE request error whick ret: %d, req str: %s", ret, TEST_GET_REQ.c_str());
		return ret;
	}
	std::string value = req.get_param("name");
	LOG_INFO("Get 'name' param : %s", value.c_str());
