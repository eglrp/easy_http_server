...
void favicons(request& req, response& res)
{
	res.set_head("Content-Type", "image/png");
	res.set_body(std::string(favicon, sizeof(favicon)));
}
...
int main(int argc, char** argv)
{
	...
	server.add_mapping("/login", login);
	server.add_mapping("/favicon", favicons);

	server.add_bind_ip("10.104.75.17");
